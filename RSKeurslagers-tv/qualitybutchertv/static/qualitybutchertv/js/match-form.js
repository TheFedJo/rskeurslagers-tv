class MatchForm {
  ...
  populate(match) {
    ...
    match.participants_detail
      .filter((p) => p.team === 1)
      .forEach((p, i) => {
        const el = document.getElementById(`t1p${i}`)
        if (el) el.value = p.player
      })
    match.participants_detail
      .filter((p) => p.team === 2)
      .forEach((p, i) => {
        const el = document.getElementById(`t2p${i}`)
        if (el) el.value = p.player
      })
  ...
  collectParticipants() {
    ...
    for (let i = 0; i < t1c; i++) {
      ...
      if (!v) {
        toast(gettext('Voeg alle spelers van team 1 toe'), 'err')
        return null
      }
      t1.push(v)
    }
    for (let i = 0; i < t2c; i++) {
      ...
      if (!v) {
        toast(gettext('Voeg alle spelers van team 2 toe'), 'err')
        return null
      }
      t2.push(v)
    }
